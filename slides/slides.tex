...
\begin{frame}{Navigate Conference Research with Agentic AI}
  ...
  \textbf{This talk:} let an AI agent do the heavy lifting --- \textit{with you in control}
\end{frame}
...
\begin{frame}{Abstracts Explorer Data Stack}
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \textbf{Retrieval}  & Keyword (SQL) + semantic (cosine similarity) search      \\
    \textbf{Reasoning}  & RAG chat: the LLM reads relevant abstracts, then answers \\
    \textbf{Agency}     & MCP tools: the LLM decides which analysis to run         \\
    \textbf{Interface}  & Flask web UI --- four tabs, no coding required           \\
    \bottomrule
  \end{tabularx}
  \vspace{0.5em}
  \begin{block}{}
    Open source $\cdot$ Apache-2.0 $\cdot$ runs locally or on a server
  \end{block}
\end{frame}
...
\begin{frame}{This Is Genuine Agency}
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \textbf{Persistent state}     & Conversation history, ratings, cluster cache across sessions                                     \\
...
\begin{frame}{Working Today for 8 Conferences}
  ...
  \begin{itemize}
    \item 8 conferences, multi-year data (NeurIPS $\cdot$ ICLR $\cdot$ ICML $\cdot$ CHI $\cdot$ HAICON $\cdot$ \ldots)
          \begin{itemize}
            \item Plugin system makes it easy to add more
          \end{itemize}
...
\begin{frame}{Summary}
  ...
  \begin{alertblock}{}
    \textbf{Agentic AI is most useful when it operates on \textit{your} domain data,\\
      in \textit{your} infrastructure, with \textit{you} steering the goals.}
  \end{alertblock}
  \vspace{0.3em}
